...
def materialize_saas_inspection(
    ...
    (writer 예외는 상위 orchestrator 에서 FAILED 로 처리 — 여기선 raise 그대로 전파.)
    """
...
def run_saas_c10_and_materialize(
    ...
    - C-10 성공 후 writer 실패 → C-10 보존, inspection_materialization.status='FAILED' (silent swallow 0; 상위 로그).
    ...
    except Exception:  # noqa: BLE001 — C-10 정본 보존, consumer projection 실패만 표기
