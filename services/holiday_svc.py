"""
holiday_svc — 조직 공용 휴무 캘린더 서비스. v1.0.0
...
여기서 휴무일을 조회하고 작업일을 판정한다.
  ...
  후속 소비자: 점검 스케줄 산정(inspection_schedule), TBM 미실시 알림, 교육 기한 산정 등
...
조회는 세 스코프를 병합한다 — 사업장 화면에는 법정공휴일과 자체 휴무가 함께 보인다.
    ...
    try:
        ...
        if company_id:
            comp = (sb.table("org_holiday").select("*")
                    ...
                    .execute().data) or []
            for r in comp:
                # 시설 스코프 행은 factory_id 일치할 때만 포함
                if r.get("factory_id") and factory_id and str(r["factory_id"]) != str(factory_id):
                    continue
                if r.get("factory_id") and not factory_id:
                    continue
                rows.append(r)
    except Exception:
        return []
