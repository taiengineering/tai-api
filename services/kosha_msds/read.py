...
from services.kosha_msds.contract import ALLOWED_SECTIONS, SECTION_MAX, SECTION_MIN
...
class MemoryMsdsReadStore:
    ...
    Callers seed `current_rows`, `chemicals` (for last_date), and
    `sections`. No insert/update/delete methods are exposed.
    """
    ...
    def _iter_current(self) -> Iterable[dict]:
        return iter(self._current)

    def get_current_by_chem_id(self, chem_id: str) -> Optional[dict]:
    ...
    def list_current(self, *, limit: int, offset: int) -> tuple[list[dict], int]:
        rows = sorted(self._current, key=lambda r: r.get("chem_id") or "")
...
class SupabaseMsdsReadStore:
    ...
    def search_current(
        ...
        *,
        ...
        q = self.sb.table(CURRENT_VIEW).select(CURRENT_VIEW_SELECT, count="exact")
        if chem_id:
...
def get_by_chem_id(chem_id: str, *, store, include_sections: bool = True) -> Optional[dict]:
    ...
    if not isinstance(chem_id, str) or not chem_id:
        return None
    row = store.get_current_by_chem_id(chem_id)
    if row is None:
        return None
...
def get_section(chem_id: str, section_no: int, *, store) -> Optional[dict]:
    """Return a single section for a CURRENT chemical, or None.

    Section number is validated first (1..16). Membership in the current
    view is checked BEFORE reading the section — a chemical that is not
    in the current view will always return None for section reads,
...
def list_current(*, store, limit: int = DEFAULT_LIMIT, offset: int = 0) -> dict:
    ...
    for the section-level payload.
    """
    lim, off = _clamp_pagination(limit, offset)
    rows, total = store.list_current(limit=lim, offset=off)
    return {
        "items": [
...
def search(
    *,
    ...
    """Simple filter search over the current view.
    ...
    rows, total = store.search_current(
        ...
        name_ko=name_ko, name_en=name_en, limit=lim, offset=off,
    )
    return {
        "items": [
            _current_row_to_contract(row, last_date=None) for row in rows
