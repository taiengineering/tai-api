...
# - POST /law-collector/collect/{law_id} 단건 수집
...
LAW_API_OC   = os.environ.get("LAW_API_OC")        # law.go.kr 인증키 (이메일 ID)
...
async def fetch_law_list(query: str, display: int = 100, page: int = 1) -> dict:
    """법령 목록 조회 API"""
...
async def fetch_law_content(mst_no: str) -> dict:
    """법령 본문 조회 API (조문 포함)"""
...
def parse_law_list_xml(xml_text: str) -> list[dict]:
    """법령 목록 XML 파싱"""
...
def parse_law_content_xml(xml_text: str) -> dict:
    ...
    if basic is not None:
        info = {
            ...
            "ministry_code":     basic.find("소관부처").get("소관부처코드", "") if basic.find("소관부처") is not None else "",
...
async def save_law_to_db(law_info: dict, raw_xml: str, articles: list, supabase) -> dict:
    """법령 전체를 DB에 저장"""
...
async def check_law_update(law_tracking: dict, supabase) -> dict:
    """단일 법령 변경 감지"""
    ...
    if current_mst_no == last_mst_no:
        # law_update_tracking 최종확인 시간 업데이트
    ...
    # 변경 감지됨 → 본문 수집
    content_result = await fetch_law_content(current_mst_no)
    ...
    save_result = await save_law_to_db(law_info, content_result["xml"], parsed["articles"], supabase)
...
async def collect_all_laws(background_tasks: BackgroundTasks):
    """
    전체 법령 수집 (최초 1회 실행)
    law_collection_target 테이블의 모든 법령을 순차 수집
    """
    background_tasks.add_task(_run_collect_all)
    return {
        "status": "started",
        "message": "전체 법령 수집이 백그라운드에서 시작됐습니다. /law-collector/status 에서 진행 상황을 확인하세요."
    }
...
async def _run_collect_all():
    ...
    for target in targets.data:
        try:
            # API로 법령 검색
            list_result = await fetch_law_list(query=target["law_name"], display=5)
            laws = parse_law_list_xml(list_result["xml"])
            ...
            # 본문 수집
            content_result = await fetch_law_content(matched["law_mst_no"])
            parsed = parse_law_content_xml(content_result["xml"])
            ...
            await save_law_to_db(law_info, content_result["xml"], parsed["articles"], supabase)
            results["success"] += 1
...
async def check_all_updates(background_tasks: BackgroundTasks):
    """
    전체 변경 감지 (증분 수집 - 스케줄러에서 호출)
    law_update_tracking 의 모든 법령 변경 여부 확인
    """
...
async def _run_check_updates():
    """변경 감지 백그라운드 작업"""
    supabase = get_supabase()
    trackings = supabase.table("law_update_tracking")\
        .select("*")\
        .execute()
    ...
    for tracking in trackings.data:
        try:
            result = await check_law_update(tracking, supabase)
            results["checked"] += 1
