"""
routers/weather.py — v1.0.4

기상청 API Hub 연동 날씨 라우터

【401 근본 원인 해결】
  기존 코드: apis.data.go.kr URL + serviceKey 파라미터
  실제 키:   apihub.kma.go.kr 의 authKey
  → URL을 apihub.kma.go.kr 로 변경 + 파라미터를 authKey 로 변경
...
환경변수:
  KMA_SERVICE_KEY   기상청 API Hub 인증키 (apihub.kma.go.kr 발급)
"""
...
# ── 기상청 API Hub URL ────────────────────────────────────────────────────
# data.go.kr (apis.data.go.kr) 아님 — apihub.kma.go.kr 전용
KMA_HUB   = "https://apihub.kma.go.kr/api/typ02/openApi"
ULTRA_URL = f"{KMA_HUB}/VilageFcstInfoService_2.0/getUltraSrtNcst"  # 초단기실황
WARN_URL  = f"{KMA_HUB}/WarningInfoService/getWthrWrnList"           # 기상특보
...
async def _hub_get(base_url: str, params: dict) -> httpx.Response:
    """
    API Hub 요청: authKey를 URL에 직접 포함, 나머지는 urlencode.
    API Hub 인증: ?authKey=KEY&... 형식 (serviceKey 아님)
    """
    key = params.pop("authKey")
    qs  = urlencode(params)
    url = f"{base_url}?authKey={key}&{qs}"
...
async def get_weather_now(
    ...
    """현재 날씨 + 작업중지 판단 (기상청 API Hub 초단기실황)"""
    key = _auth_key()
    ...
    if resp.status_code != 200:
        ...
        raise HTTPException(status_code=502,
                            detail=f"기상청 API Hub HTTP {resp.status_code}: {err}")
    ...
    if not items:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"기상청 응답 없음 (resultCode={rc}, {base_date} {base_time})"
        )
    ...
    return {
        ...
        "work_stop": {
            ...
            "triggered": triggered,
            "message":   (
                f"작업중지 필요: {', '.join(t['name'] for t in triggered)}"
                if work_stop else "작업 진행 가능"
            ),
        },
    }
...
async def get_weather_alert(
    ...
    for item in items:
        ...
        alerts.append({
            ...
            "work_stop_related": wtype in WORK_STOP_WARN,
        })
