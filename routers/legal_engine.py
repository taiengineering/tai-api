...
# ══════════════════════════════════════════════
# 3단계 진단 API (v4.2.0 신규)
# ══════════════════════════════════════════════

VALID_SECTORS = ('BUILDING', 'MANUFACTURING', 'CONSTRUCTION', 'SPECIAL_FACILITY')
...
def diagnose_step1(body: dict):
    ...
    {
      "factory_id": "uuid",
      "sector": "BUILDING",           # BUILDING/MANUFACTURING/CONSTRUCTION/SPECIAL_FACILITY
      "input": {
    ...
    sector     = body.get('sector', 'BUILDING')
    input_data = body.get('input', {})

    if not factory_id:
        raise HTTPException(status_code=400, detail='factory_id 필수')
    if sector not in VALID_SECTORS:
        raise HTTPException(status_code=400, detail=f'sector는 {VALID_SECTORS} 중 하나여야 합니다')
    ...
    diagnosis = _save_diagnosis_result(supabase, factory_id, sector, 1, input_data, matched)
