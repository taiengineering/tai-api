...
GPT API로 주어 판정 → 4중 그물 검증 → 통과분만 executor_fixed에 기록.

규정: docs/2026-06-14_LLM_EXECUTOR_RULES_VALIDATION.md (v2, 4중그물)
- LLM은 원문서 주어 발췌만(생성 금지). 주어없음/의무아님 허용.
- 4중그물: ①Kiwi 명사검증 ②원문 발췌존재 ③BLOCKLIST ④role_check(실행주체만).
...
_SYSTEM_PROMPT = (
    ...
    "3) 의무 자체가 아니면(벌칙 '처한다/과태료', 위임 '정한다', 준용 '준용한다', "
    "효력 '소멸한다/상실한다', 정의) executor='의무아님'.\n"
    "4) 법령형식어(대통령령/부령/조례/고시)는 주어가 아니다.\n"
    "5) 'X가 ~하는/~된/~인' 수식절의 X는 수식절 주어일 뿐 의무 주체가 아니다.\n"
    "6) 불확실하면 executor='주어없음', confidence='low'.\n"
    "출력은 JSON만. 형식: "
...
def _passes_nets(executor: str, source_part: str, role_check: str) -> tuple[bool, str]:
    """4중 그물. 통과 여부 + 사유."""
...
def executor_llm_fix(
    ...
    sb = get_supabase()
    # DEFER_REVIEW 잔여를 limit개 (이미 LLM 시도한 것 제외: review_reason에 마커)
    rows = (
    ...
    for r in rows:
        ...
        ok, reason = _passes_nets(executor, pt, role)
        ...
        if ok and conf == "high":
            ...
            if executor in ("주어없음", "의무아님"):
                results["non_subject"] += 1
                if not dry_run:
                    sb.table("semantic_clause_fix").update({
                        "review_reason": f"GPT 판정: {executor}",
                    }).eq("id", rid).execute()
            else:
                results["rejected"] += 1
