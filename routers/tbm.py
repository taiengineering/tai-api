...
def create_tbm(body: TbmCreateBody):
    ...
    return {"status": "success", "message": "TBM이 생성됐습니다.", "data": res.data[0]}
...
def list_tbm(
    ...
    q = supabase.table("tbm_meetings").select(
        "id, factory_id, company_id, meeting_title, work_date, work_location, "
        "conductor_name, attendee_count, status_code, transcript_status, "
        "audio_url, completed_at, created_at",
        count="exact"
    )
...
def add_attendee(tbm_id: str, body: AttendeeAddBody):
    ...
    return {"status": "success", "message": "참석자가 추가됐습니다.", "data": res.data[0]}
...
def register_sign(tbm_id: str, attendee_id: str, body: SignBody):
    ...
    return {"status": "success", "message": "서명이 등록됐습니다.", "data": res.data[0]}
