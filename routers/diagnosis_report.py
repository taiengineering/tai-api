"""
routers/diagnosis_report.py — v1.0.0

유료 진단 상세 PDF 생성 엔드포인트
  GET /diagnosis/report-pdf/{public_token}
    → anonymous_diagnosis_results에서 full_result + input_data 조회
    → tier_code PAID 티어만 허용 (무료 tier 차단)
    → 법률별 그룹핑 전처리
    → Jinja2 → diagnosis_report_paid.html 렌더링
    → xhtml2pdf → PDF bytes
    → Content-Disposition: attachment; filename=diagnosis_report_{token[:8]}.pdf
    → Content-Type: application/pdf 반환

v1.0.0 (2026-04-18):
  - 이중 페르소나 설계 (결재권자 p1~9 / 안전관리자 전체+부록)
  - 결론(p9) = SaaS 안내 (전환 포인트)
  - 부록 A: 전체 rules_table (실무 관리 도구)
  - 부록 B: inspection_required 37건
  - 부록 C: appointment_required 8건
  - 부록 D: 용어 + 참고법령
  - inspection_cycle: DB 정규화된 한글 라벨 그대로 사용 ("2년마다" 등)
  - law_groups: 건수 내림차순 상위 10개 법령만 본문에 수록
"""
...
def get_paid_report_pdf(public_token: str):
    ...
    - PAID tier_code만 허용 (무료 tier_code → 402)
    - anonymous_diagnosis_results.full_result에서 rules_table 추출
    - 법률별 그룹핑 (건수 내림차순 상위 10개 본문, 전체 부록)
    - Jinja2 렌더링 → xhtml2pdf → PDF 반환
    - 결과 URL은 Supabase Storage에 저장하지 않음 (온디맨드 생성)
    """
    ...
    appointment_raw = (
        ...
        or [r for r in rules_raw if r.get("obligation_type") == "APPOINT"]
    )
    inspection_raw = (
        full_result.get("inspection_required")
        or [r for r in rules_raw if r.get("obligation_type") == "INSPECT"]
    )
