...
def generate(active_doc, iso_doc, view_doc) -> tuple[str, str]:
    ...
    up_parts = [
        ...
        *drops,
        f"-- {len(alters_up)} ALTER TYPE timestamptz USING AT TIME ZONE 'Asia/Seoul'",
        *alters_up,
    ...
    down_parts = [
        ...
        "-- 3 view DROP (explicit, CASCADE 금지)",
        *drops,
        f"-- {len(alters_down)} ALTER TYPE timestamp without time zone USING AT TIME ZONE 'Asia/Seoul'",
        *alters_down,
        "-- 3 view EXACT recreate + OWNER / GRANT ALL / COMMENT",
        *creates,
        *owners,
