"""WO-RISK-KOSHA-B04-EVIDENCE-001 B04 mechanical evidence + B01/B02/B03 regression."""
...
GENERATOR = Path("tools/risk_map/kosha_b04_semantic_evidence.py")
EVIDENCE_TSV = Path("docs/knowledge/risk/RISK_KOSHA_B04_SEMANTIC_EVIDENCE_v1.tsv")
REPORT_MD = Path("docs/knowledge/risk/OBJ_risk-kosha-b04-semantic-evidence_v1.md")
...
def test_expected_constants():
    assert b04.BATCH_ID == "B04"
    assert b04.EXPECTED_B04_ROWS == 100
    assert b04.EXPECTED_B04_EXACT_NAME == 0
    assert b04.EXPECTED_B04_SEMANTIC_SEARCH == 100
    assert b04.B04_EVIDENCE_FIELDS == b01.B01_EVIDENCE_FIELDS


def test_b04_uses_shared_batch_helper():
...
def test_b04_deterministic_and_shape():
    rows_a, ref_a = b04.build_b04_evidence()
    rows_b, ref_b = b04.build_b04_evidence()
    assert rows_a == rows_b
    assert ref_a == ref_b
    assert b04.b04_evidence_sha(rows_a) == b04.b04_evidence_sha(rows_b) == FROZEN_B04_EVIDENCE_SHA
    assert len(rows_a) == 100
    assert len({r["review_key"] for r in rows_a}) == 100
    assert len({r["source_key"] for r in rows_a}) == 100


def test_b04_composition_is_all_semantic_search():
...
def test_b04_gpt_decision_fields_blank():
    rows, _ = b04.build_b04_evidence()
    for r in rows:
...
def test_b04_candidates_are_tasks_and_capped():
    rows, ref = b04.build_b04_evidence()
    valid = {r["canonical_id"] for r in ref}
    for r in rows:
        n = int(r["mechanical_candidate_count"])
        assert 0 <= n <= b04.CANDIDATE_CAP
        prev_score = float("inf")
        for i in range(1, b04.CANDIDATE_CAP + 1):
...
def test_b04_no_premature_decisions():
    rows, _ = b04.build_b04_evidence()
...
def test_b04_batch_boundary_exact_set_equality():
    """WO §22: B04 evidence source_key set equals frozen review-pack B04 set,
    with zero overlap against B03 and B05."""
    pack = load_tsv(GPT_REVIEW_PACK_PATH)
    b04_pack_keys = {r["source_key"] for r in pack if r["batch_no"] == "B04"}
    b03_pack_keys = {r["source_key"] for r in pack if r["batch_no"] == "B03"}
    b05_pack_keys = {r["source_key"] for r in pack if r["batch_no"] == "B05"}

    rows, _ = b04.build_b04_evidence()
    ev_keys = {r["source_key"] for r in rows}

    assert ev_keys == b04_pack_keys
    assert not (ev_keys & b03_pack_keys)
    assert not (ev_keys & b05_pack_keys)


def test_b04_source_side_columns_preserved_from_pack():
    pack = [r for r in load_tsv(GPT_REVIEW_PACK_PATH) if r["batch_no"] == "B04"]
...
# ---------------------------------------------------------------------------
# B01/B02/B03 regression — must survive B04 addition unchanged.
# ---------------------------------------------------------------------------
...
def test_b01_b02_b03_gpt_review_files_unchanged():
    ...
    b03_review = load_tsv(
        Path("docs/knowledge/risk/RISK_KOSHA_B03_GPT_SEMANTIC_REVIEW_v1.tsv")
    )
    ...
    assert universe_sha(b03_review, *DECISION_FIELDS) == FROZEN_B03_GPT_REVIEW_SHA
...
def test_written_b04_evidence_when_present():
    if not EVIDENCE_TSV.exists():
        return
    rows = load_tsv(EVIDENCE_TSV)
    assert len(rows) == 100
    assert list(rows[0].keys()) == list(b04.B04_EVIDENCE_FIELDS)
