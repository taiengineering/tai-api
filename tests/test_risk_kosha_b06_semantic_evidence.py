"""WO-RISK-KOSHA-B06-EVIDENCE-001 B06 mechanical evidence + B01-B05 regression."""
...
GENERATOR = Path("tools/risk_map/kosha_b06_semantic_evidence.py")
EVIDENCE_TSV = Path("docs/knowledge/risk/RISK_KOSHA_B06_SEMANTIC_EVIDENCE_v1.tsv")
REPORT_MD = Path("docs/knowledge/risk/OBJ_risk-kosha-b06-semantic-evidence_v1.md")
...
def test_expected_constants():
    assert b06.BATCH_ID == "B06"
    assert b06.EXPECTED_B06_ROWS == 100
    assert b06.EXPECTED_B06_EXACT_NAME == 0
    assert b06.EXPECTED_B06_SEMANTIC_SEARCH == 100
    assert b06.B06_EVIDENCE_FIELDS == b01.B01_EVIDENCE_FIELDS


def test_b06_uses_shared_batch_helper():
...
def test_b06_deterministic_and_shape():
    rows_a, ref_a = b06.build_b06_evidence()
    rows_b, ref_b = b06.build_b06_evidence()
    assert rows_a == rows_b
    assert ref_a == ref_b
    assert b06.b06_evidence_sha(rows_a) == b06.b06_evidence_sha(rows_b) == FROZEN_B06_EVIDENCE_SHA
    assert len(rows_a) == 100
    assert len({r["review_key"] for r in rows_a}) == 100
    assert len({r["source_key"] for r in rows_a}) == 100


def test_b06_composition_is_all_semantic_search():
...
def test_b06_gpt_decision_fields_blank():
    rows, _ = b06.build_b06_evidence()
    for r in rows:
...
def test_b06_candidates_are_tasks_and_capped():
    rows, ref = b06.build_b06_evidence()
    valid = {r["canonical_id"] for r in ref}
    for r in rows:
        n = int(r["mechanical_candidate_count"])
        assert 0 <= n <= b06.CANDIDATE_CAP
        prev_score = float("inf")
        for i in range(1, b06.CANDIDATE_CAP + 1):
...
def test_b06_no_premature_decisions():
    rows, _ = b06.build_b06_evidence()
...
def test_b06_batch_boundary_exact_set_equality():
    """WO §5: B06 evidence source_key set equals frozen review-pack B06 set,
    with zero overlap against B05 and B07."""
    ...
    b07_pack_keys = {r["source_key"] for r in pack if r["batch_no"] == "B07"}
    rows, _ = b06.build_b06_evidence()
    ev_keys = {r["source_key"] for r in rows}
    assert ev_keys == b06_pack_keys
    assert not (ev_keys & b05_pack_keys)
    assert not (ev_keys & b07_pack_keys)


def test_b06_source_side_columns_preserved_from_pack():
    pack = [r for r in load_tsv(GPT_REVIEW_PACK_PATH) if r["batch_no"] == "B06"]
...
# ---------------------------------------------------------------------------
# B01-B05 regression — must survive B06 addition unchanged.
# ---------------------------------------------------------------------------
...
def test_b01_b05_gpt_review_files_unchanged():
...
def test_written_b06_evidence_when_present():
    if not EVIDENCE_TSV.exists():
        return
    rows = load_tsv(EVIDENCE_TSV)
    assert len(rows) == 100
    assert list(rows[0].keys()) == list(b06.B06_EVIDENCE_FIELDS)
