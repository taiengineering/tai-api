...
def test_T1_vocabulary_len_and_distinct():
    """T1: len(_LEG_INPUT_FIELDS) == 184 && distinct == 184 (+3 pressure_vessel Frozen evidence leaves)."""
    assert len(_LEG_INPUT_FIELDS) == 184
    assert len(set(_LEG_INPUT_FIELDS)) == 184
    ...
    assert "uses_acetylene_welding_equipment" in _LEG_INPUT_FIELDS
