"""WO-RISK-04-REVIEW-005E LEAF 004E compact GPT pack. No semantic decision."""
...
from tools.risk04.review005e_pack import (
    ...
    PACK_004E_PATH,
    assert_frozen_004e,
    build_compact_pack_004e,
    parent_groups,
    render_parent_groups_004e,
)
from tools.risk04.review_decisions import GPT_MANIFEST_PATH, load_tsv
from tools.risk04.seed_review import universe_sha

COMPACT_SHA = "4da3ae49ff00597b24118fb7421d8ca724fc83e7ba80798a802cb2a0772a3bd5"
BATCH_SHAS = {
...
def test_frozen_004e_input_unchanged():
    rows = assert_frozen_004e()
    assert universe_sha(rows, *LEAF_FIELDS) == FROZEN_004E_SHA
    assert INPUT_004E_PATH.exists()
    ...
    assert [int(row["review_no"]) for row in rows][0] == 1492
    assert [int(row["review_no"]) for row in rows][-1] == 1691
...
def test_compact_pack_identity_and_pending():
    ...
    first = build_compact_pack_004e(source)
    second = build_compact_pack_004e(source)
    assert compact_pack_sha(first) == compact_pack_sha(second) == COMPACT_SHA
    g1 = render_parent_groups_004e(parent_groups(first))
    g2 = render_parent_groups_004e(parent_groups(second))
    assert g1 == g2
    assert GROUPS_004E_PATH.read_text(encoding="utf-8") == g1


def test_reviewed_counts_unchanged_and_no_004e_decisions():
    rows = []
    ...
    pack_keys = {row["source_key"] for row in load_tsv(PACK_004E_PATH)}
...
def test_review005e_does_not_decide_or_propagate():
    src = Path("tools/risk04/review005e_pack.py").read_text(encoding="utf-8")
    ...
    groups = GROUPS_004E_PATH.read_text(encoding="utf-8")
