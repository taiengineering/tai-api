"""WO-RISK-KOSHA-B05-EVIDENCE-001 KOSHA B05 semantic review evidence pack.
...
B01/B02/B03/B04. No LLM, no fuzzy, no embedding, no synonym expansion. No
semantic decision.
"""
...
WO_ID = "WO-RISK-KOSHA-B05-EVIDENCE-001"
BATCH_ID = "B05"

EXPECTED_B05_ROWS = 100
EXPECTED_B05_EXACT_NAME = 0
EXPECTED_B05_SEMANTIC_SEARCH = 100

B05_EVIDENCE_FIELDS: tuple[str, ...] = _SHARED_EVIDENCE_FIELDS

B05_EVIDENCE_PATH = Path(
    "docs/knowledge/risk/RISK_KOSHA_B05_SEMANTIC_EVIDENCE_v1.tsv"
)
REPORT_PATH = Path("docs/knowledge/risk/OBJ_risk-kosha-b05-semantic-evidence_v1.md")


def build_b05_evidence() -> tuple[list[dict], list[dict]]:
    return build_batch_evidence(
        BATCH_ID,
        EXPECTED_B05_ROWS,
        EXPECTED_B05_EXACT_NAME,
        EXPECTED_B05_SEMANTIC_SEARCH,
    )


def b05_evidence_sha(rows: list[dict]) -> str:
    return universe_sha(rows, *B05_EVIDENCE_FIELDS)
...
# {WO_ID} — KOSHA B05 Semantic Review Evidence
...
```text
RISK KOSHA B05 SEMANTIC EVIDENCE SHA = {sha_value}
```
...
def write_all() -> dict:
    rows_a, _ = build_b05_evidence()
    rows_b, _ = build_b05_evidence()
    if rows_a != rows_b:
        raise SystemExit("B05_EVIDENCE_ROW_ORDER_DRIFT")
    sha_a = b05_evidence_sha(rows_a)
    sha_b = b05_evidence_sha(rows_b)
    if sha_a != sha_b:
        raise SystemExit(f"B05_EVIDENCE_SHA_DRIFT {sha_a} vs {sha_b}")

    write_tsv(rows_a, B05_EVIDENCE_PATH, B05_EVIDENCE_FIELDS)
    ...
    return {
        ...
        "b05_evidence_sha_run1": sha_a,
        "b05_evidence_sha_run2": sha_b,
        "b05_evidence_path": str(B05_EVIDENCE_PATH),
