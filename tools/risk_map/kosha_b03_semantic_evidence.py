"""WO-RISK-KOSHA-B03-EVIDENCE-001 KOSHA B03 semantic review evidence pack.
...
B01/B02. No LLM, no fuzzy, no embedding, no synonym expansion. No semantic
decision.

Inputs (all frozen repository TSVs — same as B01/B02):
  * RISK_KOSHA_MAP001_GPT_REVIEW_PACK_v1.tsv        — filter to batch_no = B03
...
WO_ID = "WO-RISK-KOSHA-B03-EVIDENCE-001"
BATCH_ID = "B03"

# All 12 exact-name candidates live in B01, so B03 is entirely
# SEMANTIC_SEARCH_REQUIRED — same as B02.
EXPECTED_B03_ROWS = 100
EXPECTED_B03_EXACT_NAME = 0
EXPECTED_B03_SEMANTIC_SEARCH = 100

# The retrieval engine emits rows in the shared schema; B03 uses the same
# fields as B01/B02 with no schema changes.
B03_EVIDENCE_FIELDS: tuple[str, ...] = _SHARED_EVIDENCE_FIELDS

B03_EVIDENCE_PATH = Path(
    "docs/knowledge/risk/RISK_KOSHA_B03_SEMANTIC_EVIDENCE_v1.tsv"
)
REPORT_PATH = Path("docs/knowledge/risk/OBJ_risk-kosha-b03-semantic-evidence_v1.md")


def build_b03_evidence() -> tuple[list[dict], list[dict]]:
    return build_batch_evidence(
        BATCH_ID,
        EXPECTED_B03_ROWS,
        EXPECTED_B03_EXACT_NAME,
        EXPECTED_B03_SEMANTIC_SEARCH,
    )


def b03_evidence_sha(rows: list[dict]) -> str:
    return universe_sha(rows, *B03_EVIDENCE_FIELDS)
...
# {WO_ID} — KOSHA B03 Semantic Review Evidence
...
```text
RISK KOSHA B03 SEMANTIC EVIDENCE SHA = {sha_value}
```
...
def write_all() -> dict:
    rows_a, _ = build_b03_evidence()
    rows_b, _ = build_b03_evidence()
    if rows_a != rows_b:
        raise SystemExit("B03_EVIDENCE_ROW_ORDER_DRIFT")
    sha_a = b03_evidence_sha(rows_a)
    sha_b = b03_evidence_sha(rows_b)
    if sha_a != sha_b:
        raise SystemExit(f"B03_EVIDENCE_SHA_DRIFT {sha_a} vs {sha_b}")

    write_tsv(rows_a, B03_EVIDENCE_PATH, B03_EVIDENCE_FIELDS)
    ...
    return {
        ...
        "b03_evidence_sha_run1": sha_a,
        "b03_evidence_sha_run2": sha_b,
        "b03_evidence_path": str(B03_EVIDENCE_PATH),
