"""LEAF Batch004D compact GPT adjudication pack. Cursor does not decide semantic kind."""
...
FROZEN_004D_SHA = "f0cdea5c724fb5c7d66104a3fe72aa3aceaa550c9239fb5fe065eb58d3cfa047"
INPUT_004D_PATH = Path("docs/knowledge/risk/RISK04_CICW_LEAF_BATCH004D_REVIEW_INPUT.tsv")
PACK_004D_PATH = Path("docs/knowledge/risk/RISK04_CICW_LEAF_BATCH004D_GPT_REVIEW_PACK.tsv")
GROUPS_004D_PATH = Path("docs/knowledge/risk/RISK04_CICW_LEAF_BATCH004D_PARENT_GROUPS.md")
...
def assert_frozen_004d() -> list[dict]:
    rows = load_leaf_batch(INPUT_004D_PATH)
    digest = universe_sha(rows, *LEAF_FIELDS)
    if digest != FROZEN_004D_SHA:
        raise ValueError("frozen 004D input SHA mismatch")
    if len(rows) != 200:
        raise ValueError(f"004D rows {len(rows)}")
    if [int(row["review_no"]) for row in rows] != list(range(1292, 1492)):
        raise ValueError("004D review_no drift")
    if any(row["gpt_semantic_kind"] != "PENDING" for row in rows):
        raise ValueError("004D input already has GPT kind")
    return rows


def build_compact_pack_004d(rows: list[dict] | None = None) -> list[dict]:
    rows = rows if rows is not None else assert_frozen_004d()
    pack = []
    for row in rows:
        pack.append(
            {
    ...
    if [int(row["review_no"]) for row in pack] != list(range(1292, 1492)):
...
def render_parent_groups_004d(groups: list[dict]) -> str:
    lines = [
        "---",
        "class: records",
        "type: report",
        "scope: knowledge",
        "project: risk",
        "title: OBJ-RISK-04 REVIEW-005D LEAF 004D parent groups",
        "version: 1",
        "status: active",
        "owner: taiwang",
        "---",
        "",
        "# LEAF Batch004D parent group evidence",
        "",
    ...
    for group in groups:
        lines.extend(
            [
                ...
                f"004D child count = {group['child_count']}",
                f"004D review_no list = {group['review_no_list']}",
...
def write_review005d() -> dict:
    source = assert_frozen_004d()
    pack = build_compact_pack_004d(source)
    write_tsv(pack, PACK_004D_PATH, COMPACT_FIELDS)
    groups = parent_groups(pack)
    GROUPS_004D_PATH.write_text(render_parent_groups_004d(groups), encoding="utf-8")
    return {
        "rows": pack,
        "sha": compact_pack_sha(pack),
        "groups": groups,
        "input_sha": FROZEN_004D_SHA,
        "parent_groups": len(groups),
    }


def main() -> None:
    out = write_review005d()
    print(
        json.dumps(
            {
                "WO": "WO-RISK-04-REVIEW-005D",
                "004D_INPUT_SHA": out["input_sha"],
                "COMPACT_SHA": out["sha"],
                "rows": len(out["rows"]),
