"""LEAF Batch004E compact GPT adjudication pack. Cursor does not decide semantic kind."""
...
FROZEN_004E_SHA = "2a2a973666b7144045272adbcd8a3456218e83e5f5c4c876fc94480c8c20ab64"
INPUT_004E_PATH = Path("docs/knowledge/risk/RISK04_CICW_LEAF_BATCH004E_REVIEW_INPUT.tsv")
PACK_004E_PATH = Path("docs/knowledge/risk/RISK04_CICW_LEAF_BATCH004E_GPT_REVIEW_PACK.tsv")
GROUPS_004E_PATH = Path("docs/knowledge/risk/RISK04_CICW_LEAF_BATCH004E_PARENT_GROUPS.md")
...
def assert_frozen_004e() -> list[dict]:
    rows = load_leaf_batch(INPUT_004E_PATH)
    digest = universe_sha(rows, *LEAF_FIELDS)
    if digest != FROZEN_004E_SHA:
        raise ValueError("frozen 004E input SHA mismatch")
    if len(rows) != 200:
        raise ValueError(f"004E rows {len(rows)}")
    if [int(row["review_no"]) for row in rows] != list(range(1492, 1692)):
        raise ValueError("004E review_no drift")
    if any(row["gpt_semantic_kind"] != "PENDING" for row in rows):
        raise ValueError("004E input already has GPT kind")
    return rows


def build_compact_pack_004e(rows: list[dict] | None = None) -> list[dict]:
    rows = rows if rows is not None else assert_frozen_004e()
    pack = []
    for row in rows:
        pack.append(
            {
    ...
    if [int(row["review_no"]) for row in pack] != list(range(1492, 1692)):
...
def render_parent_groups_004e(groups: list[dict]) -> str:
    lines = [
        "---",
        "class: records",
        "type: report",
        "scope: knowledge",
        "project: risk",
        "title: OBJ-RISK-04 REVIEW-005E LEAF 004E parent groups",
        "version: 1",
        "status: active",
        "owner: taiwang",
        "---",
        "",
        "# LEAF Batch004E parent group evidence",
        "",
    ...
    for group in groups:
        lines.extend(
            [
                ...
                f"004E child count = {group['child_count']}",
                f"004E review_no list = {group['review_no_list']}",
...
def write_review005e() -> dict:
    source = assert_frozen_004e()
    pack = build_compact_pack_004e(source)
    write_tsv(pack, PACK_004E_PATH, COMPACT_FIELDS)
    groups = parent_groups(pack)
    GROUPS_004E_PATH.write_text(render_parent_groups_004e(groups), encoding="utf-8")
    return {
        "rows": pack,
        "sha": compact_pack_sha(pack),
        "groups": groups,
        "input_sha": FROZEN_004E_SHA,
        "parent_groups": len(groups),
    }


def main() -> None:
    out = write_review005e()
    print(
        json.dumps(
            {
                "WO": "WO-RISK-04-REVIEW-005E",
                "004E_INPUT_SHA": out["input_sha"],
                "COMPACT_SHA": out["sha"],
                "rows": len(out["rows"]),
