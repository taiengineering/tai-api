...
aud AS (
  ...
  WHERE source_id = 'dc79ac3c-388c-42dc-b029-3dd9bda54a47'
    AND action = 'PROMOTE_TO_RUNTIME_USE'
)
SELECT
  (SELECT id FROM s)                                     AS schema_id,
  (SELECT status FROM s)                                 AS schema_status,
  (SELECT bool_and(form_type='CUSTOM' AND document_family='DOCUMENT'
                   AND field_count=5 AND checklist_count=0 AND evidence_count=0) FROM s) AS schema_header_ok,
  (SELECT field_cnt FROM f)                              AS field_count,
  (SELECT field_approved_cnt FROM f)                     AS field_approved_count,
  (SELECT req_by_human_cnt FROM f)                       AS field_required_by_human_count,
  (SELECT not_required_cnt FROM f)                       AS field_not_required_count,
  (SELECT candidate_only_cnt FROM f)                     AS field_candidate_only_count,
  (SELECT exact_field_contract_cnt FROM f)               AS exact_field_contract_count,
  (SELECT status FROM document_schema_candidate WHERE form_code='GEN-INSPECT-RESULT-001') AS candidate_status,
  (SELECT count(*) FROM runtime_inspection_bridge WHERE runtime_form_schema_id='dc79ac3c-388c-42dc-b029-3dd9bda54a47') AS bridge_ref_count,
  (SELECT count(*) FROM runtime_document_data WHERE form_schema_id='dc79ac3c-388c-42dc-b029-3dd9bda54a47') AS runtime_document_ref_count,
  (SELECT audit_cnt FROM aud)                            AS audit_log_count,
  CASE WHEN
  ...
  THEN 'PASS' ELSE 'FAIL' END                            AS promotion_state_gate_status;
