...
## doc_rule_mapping 재구성

### 변화
| 항목 | 이전 | 이후 |
|---|---|---|
| `doc_rule_mapping` (운영) | 616 | **169** (재구성) |
...
2. `(parsed_law_name, parsed_article)` 추출
3. master.(law_name, law_article) 매칭
4. law_alias로 약칭 변환 (산안법 → 산업안전보건법 등)
5. 공백 정규화
...
- 문서가 참조하는 법령 265종
- 매핑 안 된 법령들:
  - 조문번호 없는 law_ref: 143건 (54%)
  - master에 없는 법령: 97건
  - 모호 표기 ("법", "시행규칙", "시행령" 단독)

### 다음 세션 분석 필요
- 매핑 안 된 181 문서의 law_ref 패턴
- master에 추가 정제 필요한 법령 식별
- document_forms.required_fields의 law_ref 보강
...
### 즉시 진행
1. **매핑 안 된 70% 문서 분석** — 어떤 법령이 누락됐는지 추적
...
WITH law_ref_parsed AS (
  SELECT 
    df.doc_id,
    rf->>'field_key' as field_key,
    rf->>'field_name' as field_name,
...
law_ref_resolved AS (
  SELECT 
    lrp.doc_id, lrp.field_key, lrp.field_name, lrp.law_ref, lrp.parsed_article,
    COALESCE(
      (SELECT m.law_name FROM master_building_legal_rules m 
       WHERE m.law_name = lrp.parsed_law_name LIMIT 1),
      (SELECT la.full_name FROM law_alias la 
       WHERE la.short_name = lrp.parsed_law_name LIMIT 1),
      (SELECT m.law_name FROM master_building_legal_rules m 
...
matched AS (
  ...
  JOIN master_building_legal_rules m
    ON m.law_name = lrr.resolved_law_name
    AND m.law_article = lrr.parsed_article
